...
double *compute_qft(double *array, int n_qft_point, int n) {
  ...
  int i;
  // printf("expstart\n");
  std::complex<double> im(0, -n); 
  for (i = 0; i <len_nums_complex; i++) 
  {
    nums_complex[i] = im+(double)n;// exp(2*pi*array[i])+(double)n;
...
int main(int argc, char** argv) {
  ...
  //           << v2 << '\n'
  //           <<  outer_prod(v1, v2)<< '\n';
  int num_point_qft = 3;//atoi(argv[1]);
